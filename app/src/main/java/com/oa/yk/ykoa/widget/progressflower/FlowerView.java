package com.oa.yk.ykoa.widget.progressflower;
...
import com.oa.yk.ykoa.utils.handler.WeakHandler;
...
public final class FlowerView extends View {
    ...
    private FlowerUpdateWeakHandler mHandler;
    ...
    public FlowerView(
        ...
        super(context);
        mHandler = new FlowerUpdateWeakHandler(updateHandler);
    ...
    private void init(
        ...
        mPetalLength = (int) ((1 - borderPadding) / 2 * mSize);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mText != null) {
            canvas.drawText(mText, mFinalSize / 2 - mTextWidth / 2, mSize / 2 + mPetalLength + mTextMarginTop + mTextHeight / 2, mTextPaint);
        }
    }

    private class FlowerUpdateWeakHandler extends WeakHandler {
        public FlowerUpdateWeakHandler(@Nullable Handler.Callback callback) {
            super(callback);
        }
    }

    private Handler.Callback updateHandler = new Handler.Callback() {
        @Override
